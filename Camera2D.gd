...
func _process(delta):
	self.position.x += (int(Input.is_action_pressed("camera_pan_right")) - int(Input.is_action_pressed("camera_pan_left"))) * 500 * current_zoom * delta
	self.position.y += (int(Input.is_action_pressed("camera_pan_down")) - int(Input.is_action_pressed("camera_pan_up"))) * 500 * current_zoom * delta
	...
	if Input.is_action_pressed("mouse_middle"):
		var ref = get_viewport().get_mouse_position()
		self.position.x -= (ref.x - fixed_toggle_point.x) * current_zoom
		self.position.y -= (ref.y - fixed_toggle_point.y) * current_zoom
		fixed_toggle_point = ref
